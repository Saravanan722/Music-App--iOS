...
struct LyricsView: View {
    @ObservedObject var driver = LyricDriverViewModel(
        track: TrackInfo(
            title: "Lyrics That Flow",
            artist: "The SwiftUI Band",
            length: 245,
            lyrics: test_lyrics,
            times: test_times
        )
            ...
            ScrollViewReader { proxy in
                ScrollView(.vertical, showsIndicators: false) {
                    VStack {
                        ...
                        ForEach(driver.songLines, id: \.keyTime) { line in
                            HStack {
                                Text(line.line)
                                    .font(.title)
                    ...
                    .onAppear {
                        driver.startTimer()
                        driver.parseLyrics(driver.lyricsURL)
//                        let parsedLyrics = driver.parseLyrics(driver.lyricsString)
//                        print(parsedLyrics)
//                        print(driver.songLines)
                    }
                }
                .animation(.linear, value: driver.time)
                .onChange(of: driver.time) { newTime in
//                    print(newTime)
